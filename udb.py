...
class UDB :
    ...
    def addAgent(self,agent):
        ...
        if agent.parent is not None:
            index = self.agent_sub_index[agent.parent]
            if index is None:
                index = Set()
    ...
    def addCompany(self,company):
        ...
        if company.parent is not None:
            index = self.company_sub_index[company.parent]
            if index is None:
                index = Set()
        ...
        index = self.agent_company_index[company.agent]
        if index is None:
            index = Set()
    ...
    def addUser(self,user):
        ...
        index = self.company_user_index[user.company]
        if index is None:
            index = Set()
            self.company_user_index[user.company] = index
        index.add(user.uid)

        company = self.companys[user.company]
        if company is None:
            logging.error('User %d \'s company %d is not exists' % (user.uid,user.company))
            return
        if company.agent is None:
            return
        index = self.agent_user_index[company.agent]
        if index is None:
            index = Set()
    ...
    def addUserToGroup(self,uid,gid):
        index = self.user_group_index[uid]
        if index is None:
            index = Set()
            self.user_group_index[uid] = index
        index.add(gid)

        index = self.group_user_index[gid]
        if index is None:
            index = Set()
